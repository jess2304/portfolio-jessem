...
export default {
  ...
  methods: {
    toggleFormation(index) {
      this.formations[index].detailsVisible = !this.formations[index].detailsVisible
...
<template>
  <div class="formations-container" id="formations">
    <h1>Formations</h1>
    <div v-for="(formation, index) in formations" :key="index" class="formation" :data-aos="'fade-up'" :data-aos-delay="index * 100">
      ...
      <ul v-if="formation.detailsVisible">
        <li v-for="matiere in formation.matieres" :key="matiere">{{ matiere }}</li>
      </ul>
    </div>
  </div>
</template>
...
.formation:hover h2, .formation:hover h3 {
  color: #007bff;
}
